...
class ResxFileManager implements TranslationFileManager {
	 ...
	 * Source translations.
	 */
	private _source: JSDOM;

	/**
	 * Cache of the original translations.
	 */
	...
	constructor(source: JSDOM) {
		this._source = source;
		this._translations = source.window.document
			.querySelectorAll("root > data > value")
			.values()
	...
	public async save(path: string, translations: string[]): Promise<void> {
		const clone = this._source.window.document.cloneNode(true) as unknown as Document;

		const arr: Element[] = [];
		let i = 0;

		for (const data of clone.querySelectorAll("root > data")) {
			const values = data.querySelectorAll("value");
			for (const value of values) {
				// Must mimic the filter in the constructor to match indexes.
				if (value && value.textContent) {
					value.textContent = translations[i++] ?? null;
				}
			}
		...
		await write(path, this.serialize(arr));
	}
	...
	/**
	 * Serializes the elements to RESW format.
	 * @param elements Elements to serialize.
	 * @returns Serialized contents.
	 */
	private serialize(elements: Element[]): string {
		const data = elements.reduce((prev, el) => `${prev}  ${el.outerHTML}\n`, "").trimEnd();
